...
const Explore = () => {
  ...
  const { data: searchResultPost, isFetching: isSearchFetching } =
    useSearchPosts(debounceValue);
  console.log(searchResultPost);
  useEffect(() => {
  ...
  return (
    <div className="explore-container">
      <div className="explore-inner_container">
        <h3 className="w-full capitalize h3-bold md:h2-bold">search post</h3>
        <div className="flex w-full gap-1 pl-2 rounded-lg bg-dark-4 ">
          ...
          <Input
            ...
            className="explore-search placeholder:text-xl"
          ></Input>
        </div>
      </div>
      <div className="flex w-full max-w-5xl mt-16 mb-7 flex-between">
        <h3 className="capitalize body-bold md:h3:bold">popular today</h3>
        <div className="gap-2 px-4 py-3 cursor-pointer flex-center bg-dark-3 rounded-xl ">
          <p className="uppercase small-medium md:base-medium text-light-2">
            all
          </p>
      ...
      <div className="flex flex-wrap w-full max-w-5xl gap-9">
        {shouldShowSearchResults ? (
          <SearchResult
            ...
            searchedPosts={searchResultPost}
          />
        ) : shouldShowPosts ? (
